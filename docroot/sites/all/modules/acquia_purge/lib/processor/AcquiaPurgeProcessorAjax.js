...
(function ($) {
  Drupal.behaviors.AcquiaPurgeProcessorAjax = {
    attach: function (context) {
      $(document).ready(function() {
        ...
        function eventLoopRun() {
          ...
          $.ajax({
            ...
            error: function(request, textStatus) {
              ...
              // However, if there's a parser error, that indicates a more serious error.
              // Don't attempt to re-try in that case to avoid a potential infinite loop.
              if (Number(request['status']) == 200 && textStatus !== 'parsererror') {
                if (uiActivated()) {
                  uiThrobberOff();
                }
                eventLoopRun();
        ...
        function quit() {
          delete Drupal.behaviors.AcquiaPurgeProcessorAjax;
        }
